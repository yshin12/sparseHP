...
lockdown = "2020-01-23" # Jan 23: lockdown date
ghat = 1/18             # gamma
...
early_end_date = min(which((movavg(df$confirmed, 3, type="s")<10)&(cumsum(df$confirmed)>100)))   # When the 3-day average newly confirmed cases is less than equal to 10 for the first time
df = df[1:early_end_date,]
