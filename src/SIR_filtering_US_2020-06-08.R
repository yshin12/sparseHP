...
if  (selection_L2L0c == 1){
    ...
    l0_results = l0tfc(y=ly,l0constraint=l0constraint,M=M,l2penalty=l2penalty)
       ...
       betaHat = l0_est[1:length(ly)]
        ...
        if (draw_L2L0c_L1_togher==0) {
         draw_fit_R0(data, SHP_kink_dates, country.name, file_name)
         draw_resid_R0(data, country.name, file_name)
         draw_fit_beta(data, SHP_kink_dates, country.name, file_name)
  ...
  if  (selection_L1 == 1){
    ...
    if (tuning_selection == 0){
      ld = 0.9                 # User can set the tuning parameter lambda
    }
    else{
      ld_set = seq(0,10,0.1)
    ...
    L1_filter_Rresid = R - L1_filter_Rhat
    ...
    data = data.frame(
      ...
      filter_Rhat = L1_filter_Rhat,  # fitted value, R_t scale
      filter_resid = L1_filter_resid, # residuals, log scale
      filter_Rresid =L1_filter_Rresid# residuals, R_t scale
    )

    draw_fit_R0(data, L1_kink_dates, country.name, file_name)
    draw_resid_R0(data, country.name, file_name)
    draw_fit_beta(data, L1_kink_dates, country.name, file_name)
    draw_resid_beta(data, country.name, file_name)
...
if  (selection_L2 == 1){
  ...
  draw_fit_R0(data, NULL, country.name, file_name)
  draw_resid_R0(data, country.name, file_name)
  draw_fit_beta(data, NULL , country.name, file_name)
  draw_resid_beta(data, country.name, file_name)
...
if  (selection_SQRT_L1 == 1){
  ...
  # Set the tuning parameters
  if (tuning_selection == 0){
    ld = 0.5                  # User can set the tuning parameter lambda
  }
  else{
    ld_set = seq(0,10,0.1)
  ...
  kink_dates =  date[abs(diff(x=betaHat, differences=2))>1e-6] + 1
  cat('\n\n Kink dates: \n')
  cat('---------------------------------------------------------------- \n')
  print(kink_dates)
  cat('---------------------------------------------------------------- \n')
  ...
  draw_fit_R0(data, kink_dates, country.name, file_name)
  draw_resid_R0(data, country.name, file_name)
  draw_fit_beta(data, kink_dates, country.name, file_name)
  draw_resid_beta(data, country.name, file_name)
}
